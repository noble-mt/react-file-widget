...
const PageSelectorContainer = styled.div<{ vertical: boolean }>(
  (props) => `
...
const PageWrapper = styled.div<{ vertical: boolean }>(
  (props) => `
    display: flex;
...
export const PageSelector = ({ totalPages, goToPage }: { goToPage: (page: number) => void; totalPages: number }) => {
  ...
  const vertical =
    config?.pdfProps?.pageSelectorPosition !== 'top' && config?.pdfProps?.pageSelectorPosition !== 'bottom';
  return (
    <PageSelectorContainer vertical={vertical}>
      <Document file={file?.url ?? file?.file} loading={<span>Loading...</span>}>
